...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{ route('prof.compte.updateInfo') }}" autocomplete="off" class="form-horizontal">
          @csrf
          @method('put')
                ...
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email"
                      id="input-email" type="email" placeholder="{{ __('Votre E-mail') }}"
                      value="{{ old('email', auth()->user()->Email) }}" required="true" aria-required="true" />
                    @if ($errors->has('email'))
              ...
              <div class="row">
                ...
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('adress') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('adress') ? ' is-invalid' : '' }}" name="adress"
                      id="input-adress" type="text" placeholder="{{ __('Votre adresse...') }}"
                      value="{{ old('adress', auth()->user()->Adresse) }}" required="true" aria-required="true" />
                    @if ($errors->has('adress'))
                    <span id="adress-error" class="error text-danger"
                      for="input-adress">{{ $errors->first('adress') }}</span>
                    @endif
                  </div>
                </div>
              </div>
    ...
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{ route('prof.compte.updatePassword') }}" class="form-horizontal">
          @csrf
          @method('put')
